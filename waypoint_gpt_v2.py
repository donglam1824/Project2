...
def refined_coverage_path_planning(map_data, start):
    ...
    while stack:
        ...
        if current == end:
            end_visit_count += 1
            print(f"Reached destination point {end} for the {end_visit_count} time")
            
            # If this is the second visit to end, stop the algorithm
            if end_visit_count == 2:
                print("Reached destination for the second time - stopping path planning")
                break
